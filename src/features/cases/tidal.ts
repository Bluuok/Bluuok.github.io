...
export function mountTidal(root: HTMLElement) {
  ...
  const reset = () => {
    ...
    if(group)group.rotation.set(0,0,0); if(dot)dot.visible=false; paint();
  };
  ...
  viewport.addEventListener('pointermove',e=>{
    ...
    const r=viewport.getBoundingClientRect();tx=((e.clientX-r.left)/r.width-.5)*.11;ty=((e.clientY-r.top)/r.height-.5)*.07;wake();
  },{passive:true,signal:abort.signal});
  for(const event of ['pointerleave','pointercancel'])viewport.addEventListener(event,()=>{tx=ty=0;wake();},{signal:abort.signal});
  document.addEventListener('visibilitychange',()=>{if(document.hidden)reset();else wake();},{signal:abort.signal});
