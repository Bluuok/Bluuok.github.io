...
export async function mountInlineSpark(root:HTMLElement){
 const perf=sceneReadiness('spark');perf.mark('request');
 ...
 try {
  ...
  perf.mark('module-and-images');root.dataset.imagesDecoded='true';
  ...
  cleanupIntro=mountIntro(root,{id:'inline-first-spark'});root.dataset.sparkState='ready';perf.mark('interactive');
