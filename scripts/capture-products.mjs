// Capture actual product frontends in disposable workspaces. Test identities,
// task records and model replies are synthetic; no real external account is used.
...
try {
  ...
  script += `\nfor(const name of ['chat','tasks']) { const response=await page.goto('http://127.0.0.1:5173/'+name); assert.equal(response.status(),200); await page.locator('nav').first().waitFor(); await page.waitForTimeout(800); assert.ok((await page.locator('body').innerText()).includes('Clawtide')); await page.screenshot({path:output+'/clawtide-'+name+'.jpg',type:'jpeg',quality:84}); }\nassert.equal(errors.length,0,errors.join('\\n'));await browser.close();\n`;
